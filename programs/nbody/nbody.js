function push5(a, v0, v1, v2, v3, v4) {
    a[0] = v0; a[1] = v1; a[2] = v2; a[3] = v3; a[4] = v4;
}

function offsetMomentum(x, y, z, vx, vy, vz, m) {
...
function advance(x, y, z, vx, vy, vz, m, dt) {
    for (let i = 0; i < 5; i++) {
        for (let j = i + 1; j < 5; j++) {
            const dx = x[i] - x[j];
            const dy = y[i] - y[j];
...
const x = new Array(5), y = new Array(5), z = new Array(5);
const vx = new Array(5), vy = new Array(5), vz = new Array(5), m = new Array(5);
...
const e = energy(x, y, z, vx, vy, vz, m);
console.log("nbody checksum = " + Math.trunc(e * 1000000000.0));
